...
export default function LeaderboardSeasonPoints() {
  const { playerId } = useCurrentPlayer()
  const { players: standings, loading } = usePlayerStandings()
  const [page, setPage] = useState(1)

  // Ranked by raw point total rather than scoring percentage — players who
  // haven't made any picks yet sort last with no rank, same treatment the
  // percentage leaderboard gives them.
  const players = useMemo(() => {
    const ranked = standings
      .filter(p => p.weeksPlayed > 0)
      .slice()
      .sort((a, b) => b.score - a.score)
      .map((p, i) => ({ ...p, pointsRank: i + 1 }))
    const unranked = standings.filter(p => p.weeksPlayed === 0)
    ...
    return [...ranked, ...unranked].map(p => ({
      ...
      score:    p.score,
      rank:     p.pointsRank ?? null,
      medal:    medalForRank(p.pointsRank),
  ...
  return (
    <div className="min-h-screen bg-gray-100 pb-20">
      <PageHeader title="By points" backTo="/leaderboard" />
        ...
        {currentPlayer && (
          <Card>
            ...
            <div className="flex gap-6 mt-2">
              ...
              <StatPair label="Points" value={currentPlayer.score} valueColor="text-brand-primary" />
            </div>
          </Card>
        )}
