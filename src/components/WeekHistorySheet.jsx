...
function InsightCard({ insight, loading, error }) {
  return (
    <div className="mx-4 mt-3 mb-1 rounded-card bg-brand-secondary/10 px-4 py-3 flex gap-2.5">
      <Sparkles size={18} className="text-brand-secondary shrink-0 mt-0.5" />
      {loading ? (
        <p className="text-body-1 text-gray-400">Generating your recap…</p>
      ) : error ? (
        <p className="text-body-1 text-gray-400">Couldn't load an insight for this week.</p>
      ) : (
        <p className="text-body-1 text-gray-700">{insight}</p>
      )}
    </div>
  )
}
